...
public class Main {
	...
	private static final String CSV_FILE_NAME = "optimization1RealConfs.csv";
	...
	public static void main(String[] args) {
		...
		int files = new File(DIRECTORY_PATH_REAL_CONF).list().length;// src//configurations
		int filesCounter = 0;
		...
		while (filesCounter < files) {
			...
			for (int i = 0; i < 5; i++) {
				...
				while (j < 2) {// switch white/black to move
					...
					int evaluatedConfs = main.solve(ROOT_NAME_REAL_CONF + (filesCounter + 1) + FORMAT, colorToMove);

...
public class ASPCheckersPrompter extends AbstractPrompter {
	...
	public static final int HIGHEST_MAX_DEPTH_LEVEL = 3;
	public static final int LOWEST_MAX_DEPTH_LEVEL = 3;
	public static final int MAX_DEPTH_LEVEL_THRESHOLD = 5;
	...
	public int solve() {
		...
		if (configurations.size() <= MAX_DEPTH_LEVEL_THRESHOLD)
	...
	private void simulateMatch(GameConfiguration conf, Move moveOriginator, PawnsColor color,
		...
		if (tmpConfs.size() <= MAX_DEPTH_LEVEL_THRESHOLD)
			depthEvaluation = HIGHEST_MAX_DEPTH_LEVEL;
		else
			depthEvaluation = LOWEST_MAX_DEPTH_LEVEL;

		// tmpConfs cutoff
		GameConfiguration bestBranch = getBestGameConfiguration(tmpConfs);

		simulateMatch(bestBranch, moveOriginator, PawnsColor.getOppositeColor(color), confsToEvaluate,
				countDepthLevels + 1, depthEvaluation);
